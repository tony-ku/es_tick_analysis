...
- `daily_metrics.csv` — includes **`post_ib_high`**, **`post_ib_low`**, **`post_ib_exceed_*`**, **`prior_day_vah`**, **`prior_day_val`**, **`hit_onh_or_onl`**, **`hit_prior_day_vah`**, **`hit_prior_day_val`**, **`gap_pct`**, **`gap_filled`**, **`gap_size_bucket`**
- `conditional_probabilities.csv` — 10 reference levels × 4 open buckets; **`probability_pct`** (0–100)
